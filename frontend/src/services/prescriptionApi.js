...
export const fetchDoctorPrescriptions = async (doctorId) => {
	const response = await prescriptionApi.get(`/doctor/${doctorId}`);
...
export const finalizePrescription = async (id) => {
	const response = await prescriptionApi.patch(`/${id}/finalize`);
	return response.data;
};
